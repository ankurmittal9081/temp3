...
import authMiddleware from './middleware/authMiddleware.js'; // This is verifyJWT now
...
if (!allowedOrigin) { console.error('FATAL ERROR: CORS_ORIGIN is not defined.'); process.exit(1); }
...
apiRouter.use(authMiddleware); // This is verifyJWT, protecting all routes below
...
connectDB().then(() => { app.listen(PORT, () => console.log(`🚀 Server connected to DB and running at port ${PORT}`)); }).catch(err => console.error("Failed to start server", err));