...
// The final cookie settings that work for production (cross-domain) and local (same-domain)
const COOKIE_OPTS = {
  httpOnly: true,
...
router.post('/register', async (req, res, next) => {
  ...
  try {
    // Check for existing user logic...
    const newUser = new User({ fullName, email, password, role, aadhaarNumber, ...details });
    await newUser.save();
    // Role-specific handling...
    
    // After creating user, sign a token and log them in immediately
