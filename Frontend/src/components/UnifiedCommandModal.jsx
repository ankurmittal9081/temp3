...
const UnifiedCommandModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const getIcon = () => {
    switch (currentStep?.type) {
      ...
      case "kiosk":
        return <Building className="text-orange-500" size={20} />
      case "paralegal":
        return <User className="text-cyan-500" size={20} />
      default:
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200/50 max-h-[80vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex justify-between items-center p-6 border-b border-slate-200/60">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {currentStep && (
                    <span className="text-sm font-medium text-slate-500">Creating {currentStep.type}</span>
                  )}
                </div>
              </div>
              <button 
            ...
            <div className="p-6 border-b border-slate-200/60">
              <div className="flex gap-2 p-1.5 bg-slate-100/80 rounded-xl backdrop-blur-sm">
                <button
                  onClick={() => setMode("text")}
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 transform hover:scale-105 ${
                    mode === "text" 
            ...
            {/* Conversation */}
            <div className="flex-1 p-6 overflow-y-auto space-y-4 min-h-[300px] max-h-[400px]">
              {conversation.length === 0 && (
                <div className="text-center text-slate-500 py-8">
                  ...
                  <p className="text-lg font-semibold text-slate-700 mb-2">How can I help you today?</p>
                  <p className="text-sm text-slate-500">Try: "Create issue", "Add document", "Add user", "Create kiosk", etc.</p>
                </div>
              )}
              ...
              {isProcessing && (
                <div className="flex justify-start">
                  <div className="bg-slate-100 text-slate-700 p-4 rounded-2xl flex items-center gap-3 border border-slate-200/50">
              ...
              {mode === "text" ? (
                <div className="flex gap-3">
                  <input
                    ...
                    onKeyPress={(e) => e.key === "Enter" && handleInput(input)}
                    placeholder="Type your message..."
                    className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200"
                    disabled={isProcessing}
                  />
                  <button
                    onClick={() => handleInput(input)}
                ...
                <div className="flex flex-col items-center gap-4">
                  <button
                    onClick={handleVoiceToggle}
                    disabled={isProcessing}
                    className={`w-20 h-20 rounded-2xl flex items-center justify-center text-white transition-all duration-200 transform hover:scale-110 shadow-lg ${
                      isRecording 
                  ...
                  {input && (
                    <div className="w-full p-4 bg-slate-100 border border-slate-200/50 rounded-xl text-center text-slate-700 font-medium">
                      "{input}"
                    </div>
                  )}
                </div>
